...
def draw_bbox(image, bboxes, classes=read_class_names(cfg.YOLO.CLASSES), show_label=True):
    ...
    for i in range(num_boxes[0]):
        if int(out_classes[0][i]) < 0 or int(out_classes[0][i]) > num_classes or (int(out_classes[0][i]) > 0):
            continue
        coor = out_boxes[0][i]
        ...
        c1, c2 = ((coor[1]), int(coor[0])), (int(coor[3]), int(coor[2]))
